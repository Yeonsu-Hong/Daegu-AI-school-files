...
//만약 a와 b가 갈다면 a는 b보다 작다 라는 버그가 생긴다. (a=20, b=20일때)
...
*/
//  결과는 a는 c보다 작다가 출력된다. 이유는 가장 먼저 나오는 true 만 실행한다. 뒤에꺼는 보지도 않는다.
...
// false, 외에 다른 값을 입력해도 거짓이 출력될수 있다. 빈문자열 (""), 안에 글자가 들어가면 참, 숫자 (0), 0보다 큰 경우에는 참으로 인식 , -값도 참
...
// prompt 자바스크립트에서 만들어 놓은 method 브라우저 환경에서만 동작!!
// 책 p.451
/*
...
// let,const를 가지고 변수를 만들면 지역 전역변수 기준이 달라진다.
...
//  var 키워드를 사용하지 않아서 전역변수를 지역 변수로 바꿔준것이다.
...
// window라고 하는 객체가 있다.
// consle창에서 검색하면 많은 객체들이 나온다. p.578
//  전역변수는 window라는 객체의 프러펄티로 들어가있다.
...
func1(); 
*/
//  결과는 60이 출력된다. a b c 각각을 다 출력했다. 안쪽에서 바깥쪽으로 따진다. 전역 변수
//  바깥쪽에서 안쪽으로는 안된다.
...
//  호이스팅 : 끌어 올린다는 개념 (**** 어렵다 책 보고 다시 이해!)
...
*/
// 위쪽으로 끌어 올려져도 결과는 동일하다. 문제가 전혀 없다. 
// func1();
...
// parseInt는 정수형으로 parseFloat 는 실수형으로 바꾸어줄때 사용  (string -> number로 )
